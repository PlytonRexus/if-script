...
const props = Object.keys(CACHE_VERSIONS)
props.forEach(val => {
  ...
  event.waitUntil(
    props.forEach(prop =>
      caches
        ...
        .then(cache => Object.keys(addrs).forEach(val => val.forEach(addr => cache.add(addr))))
        .catch(e => console.log(e)))
  ))

self.addEventListener('fetch', function (event) {
  const preTypes = [/\/js\//, /\/css\//, /\/images\//, /\/fonts\//]

  const index = preTypes.findIndex(preType => {
    return !!event.request.url.match(preType)
  })

  let prop = 'css'

  if (index !== -1) { prop = event.request.url.match(preTypes[index])[0].replace(/\//g, '') }
