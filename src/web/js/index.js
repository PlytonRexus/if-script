...
import instructions from 'if-script-core/test/examples/introduction.mjs'
...
.then(function () {
  ...
  function fetchFile (addr, site) {
    fetch(addr)
      ...
      .then((blob) => {
        ifscript[site] = window.URL.createObjectURL(blob)
      })
  ...
  const storyBtn = createElement(
    'a',
    {
      ...
      download: 'compiled-story.json'
    },
    null,
    {
      onclick: function () {
        if (!interpreter.run.story || Object.keys(interpreter.run.story).length <= 0) {
          return console.log('Run the story at least once.')
        }
        const data = new Blob([`${JSON.stringify(interpreter.run.story)}`], {
  ...
  const storyTextBtn = createElement(
    ...
    {
      onclick: function () {
        const data = new Blob([$('#if_r-input-area').value], {
          type: 'text/plain'
        })
        storyTextBtn.setAttribute('href', window.URL.createObjectURL(data))
      },
      innerHTML: `Download story file
  ...
  function runSubmit () {
    ...
    const story = parser(storyValue).parseStory()

    lwrite(localStorageKeys.objectStorage, JSON.stringify(story))

    interpreter.loadStory(story, null, 'choicescript-sepia')
  }
})
